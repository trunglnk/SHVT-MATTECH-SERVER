...
class ProxyHelper
{
    ...
    {
        return [
            'type' => ($this->originalRequest->isJson() ? 'json' : (strpos($this->originalRequest->header('Content-Type'), 'multipart') !== false ? 'multipart' : ($this->originalRequest->header('Content-Type') == 'application/x-www-form-urlencoded' ? 'form' : $this->originalRequest->header('Content-Type')))),
